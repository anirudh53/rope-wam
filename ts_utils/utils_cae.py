...
def load_data(
    ...
    if return_sw:
        return loadedT["den"], loadedV["den"], loadedT["F107"], loadedV["F107"], loadedT["Kp"], loadedV["Kp"]
...
def load_and_process_data(
    ...
    if return_sw:
        ...
        dataT, dataV = output
    # convert to tensor
    dataT, dataV = torch.Tensor(dataT), torch.Tensor(dataV)
    ...
    def _initialize_model(self) -> None:
        ...
        if (self._config.get("step_size", None) is not None) and (self._config.get("gamma", None) is not None):
            ...
            self._lr_scheduler = torch.optim.lr_scheduler.StepLR(
                self._optimizer, step_size=step_size, gamma=gamma, verbose=True,
            )
            ...
            for phase in ["train", "valid"]:
                ...
                for x in tqdm(dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == "train"):
                        ...
                        # compute all losses
                        loss = self._model.loss(x, xhat, latent, sample_weights=sample_weights)
                ...
                epoch_loss, epoch_METRIC = self._compile_losses(
                    running_loss,
                    running_METRIC,
                    dataset_sizes[phase],
                )
                print(
                    f"{phase} Loss: {epoch_loss:.4f} METRIC: {epoch_METRIC:.4f}"
                )
                # if validation phase
                if phase == "valid":
                    # add losses to log
